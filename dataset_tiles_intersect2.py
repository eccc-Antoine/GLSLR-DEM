    ...
    for l in liste:
        ...
        if str(l_path) in previous_list:
            ...
            if date == df_previous['modified_date'].loc[df_previous['dataset_extent_relative']==str(l_path)].values:
                ...
                print(f'dataset {str(l)} as been modified since last time, need to perform tile intersection...')
            ...
            print(f'dataset {str(l)} is new, need to perform tile intersection')
            gdf_parts, tiles=dataset_tiles_intersect(l, tiles_file)
            if dataset_name[0:3]=='SHC':                
                gdf_parts['tile']=gdf_parts['layer']
            else:
...
if __name__ == '__main__':
    tiles_file=r"F:\DEM_GLAMM\AOI\grille_Charles\Tuile_final.shp"
    workdir=r'F:\DEM_GLAMM\EXTENT_master'
    previous_version=fr"{workdir}\tiles_dataset.csv"
    main_intersect(tiles_file, workdir, previous_version)
